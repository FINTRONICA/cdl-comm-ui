...
export class AccountPurposeService {
  async getAccountPurposes(
    ...
    try {
      const result = await apiClient.get<PaginatedResponse<AccountPurpose>>(url)
  ...
  async getAccountPurpose(id: string): Promise<AccountPurpose> {
    try {
      const url = buildApiUrl(API_ENDPOINTS.MASTER_ACCOUNT_PURPOSE.GET_BY_ID(id))
  ...
  async getAllAccountPurposes(): Promise<AccountPurpose[]> {
    try {
      ...
      const result = await apiClient.get<AccountPurpose[] | PaginatedResponse<AccountPurpose>>(url)
      
      // Handle different response structures
      if (Array.isArray(result)) {
        return result
      } else if (result && typeof result === 'object' && 'content' in result && Array.isArray((result as PaginatedResponse<AccountPurpose>).content)) {
