...
const Step3 = ({ agreementFeeScheduleId: propAgreementFeeScheduleId, onEditStep, isReadOnly = false }: Step3Props) => {
  const params = useParams()
  const agreementFeeScheduleId = propAgreementFeeScheduleId || (params.id as string)
  const isDarkMode = useIsDarkMode()

  const [agreementFeeScheduleDetails, setAgreementFeeScheduleDetails] = useState<AgreementFeeSchedule | null>(
    null
  )
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      if (!agreementFeeScheduleId || agreementFeeScheduleId.trim() === '') {
        console.warn('[AgreementFeeScheduleStep3] Agreement Fee Schedule ID is missing or empty:', agreementFeeScheduleId)
        setError('Agreement Fee Schedule ID is required to load review data')
        setLoading(false)
        return
      }
      ...
      try {
        ...
        const [details, documents] = await Promise.allSettled([
          ...
          agreementFeeScheduleService.getAgreementFeeScheduleDocuments(agreementFeeScheduleId, 'AGREEMENT_FEE_SCHEDULE'),
        ])
        ...
        setAgreementFeeScheduleDetails(detailsResult as AgreementFeeSchedule)
