...
export default function AgreementStepperWrapper({
  ...
  const stepConfigs = useMemo(
    () => ({
      ...
      1: {
        validateStep: async () => {
          if (!agreementId) {
            notifications.showError(
              "Agreement ID is required to proceed to Review step."
            );
            return false;
          }
          return true;
      ...
      2: {
        validateStep: async () => {
          if (!stepStatus?.step1) {
            notifications.showError(
              "Please complete Agreement Details before continuing."
            );
            return false;
          }
          return true;
    ...
    [validateFormFields, notifications, agreementId, stepStatus?.step1]
  );
  ...
  useEffect(() => {
    if (!agreementId || !stepStatus) return;
    ...
    const querySuffix = isViewMode
      ...
      : isEditingMode
        ...
        : "";
    router.replace(
      `/agreement/${agreementId}/step/${maxAllowedStep + 1}${querySuffix}`
    );
      ...
      if (activeStep === 0) {
        const step1Data: AgreementDetailsData = {
          primaryEscrowCifNumber:
            currentFormData.primaryEscrowCifNumber as string,
          productManagerName: currentFormData.productManagerName as string,
          clientName: currentFormData.clientName as string,
          relationshipManagerName:
            currentFormData.relationshipManagerName as string,
          operatingLocationCode:
            currentFormData.operatingLocationCode as string,
          ...
          agreementParametersDTO: currentFormData.agreementParametersDTO as
            | { id: number }
            | number
            | null
            | undefined,
          agreementFeeDTO: currentFormData.agreementFeeDTO as
            | { id: number }
            | number
            | null
            | undefined,
          clientNameDTO: currentFormData.clientNameDTO as
            | { id: number }
            | number
            | null
            | undefined,
          businessSegmentDTO: currentFormData.businessSegmentDTO as
            | { id: number }
            | number
            | null
            | undefined,
          businessSubSegmentDTO: currentFormData.businessSubSegmentDTO as
            | { id: number }
            | number
            | null
            | undefined,
          dealStatusDTO: currentFormData.dealStatusDTO as
            | { id: number }
            | number
            | null
            | undefined,
          feesDTO: currentFormData.feesDTO as
            | { id: number }
            | number
            | null
            | undefined,
          dealTypeDTO: currentFormData.dealTypeDTO as
            | { id: number }
            | number
            | null
            | undefined,
          dealSubTypeDTO: currentFormData.dealSubTypeDTO as
            | { id: number }
            | number
            | null
            | undefined,
          productProgramDTO: currentFormData.productProgramDTO as
            | { id: number }
            | number
            | null
            | undefined,
          dealPriorityDTO: currentFormData.dealPriorityDTO as
            | { id: number }
            | number
            | null
            | undefined,
