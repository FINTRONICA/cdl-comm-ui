...
export default function AccountStepperWrapper({
  ...
  const stepConfigs = useMemo(
    () => ({
      ...
      2: {
        validateStep: async () => {
          if (!stepStatus?.step1) {
            notifications.showError(
              "Please complete Account Details before continuing."
            );
            return false;
          }
          return true;
    ...
    [validateFormFields, notifications, accountId, stepStatus?.step1]
  );
  ...
  useEffect(() => {
    if (!accountId || !stepStatus) return;
    ...
    const querySuffix = isViewMode
      ...
      : isEditingMode
        ...
        : "";
    router.replace(
      `/escrow-account/${accountId}/step/${maxAllowedStep + 1}${querySuffix}`
    );
