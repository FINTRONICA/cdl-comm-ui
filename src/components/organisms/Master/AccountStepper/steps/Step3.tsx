...
const Step3 = ({ accountId: propAccountId, onEditStep, isReadOnly = false }: Step3Props) => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      if (!accountId || accountId.trim() === '') {
        console.warn('[AccountStep3] Account ID is missing or empty:', accountId)
        setError('Account ID is required to load review data')
        setLoading(false)
        return
      }
      ...
      try {
        ...
        const [details, documents] = await Promise.allSettled([
          ...
          accountService.getAccountDocuments(accountId, 'ACCOUNT'),
        ])
        ...
        setAccountDetails(detailsResult as Account)
