...
const Step1 = ({ isReadOnly = false, agreementParameterId }: Step1Props) => {
      ...
      try {
        ...
        if (agreementParameter?.agreementEffectiveDate) {
          ...
          if (!currentEffectiveDate) {
            setValue('agreementEffectiveDate', agreementParameter.agreementEffectiveDate, {
              shouldValidate: true,
              shouldDirty: false,
            })
          }
        ...
        if (agreementParameter?.agreementExpiryDate) {
          ...
          if (!currentExpiryDate) {
            setValue('agreementExpiryDate', agreementParameter.agreementExpiryDate, {
              shouldValidate: true,
              shouldDirty: false,
            })
          }
        ...
        if (agreementParameter?.permittedInvestmentAllowedDTO) {
          const currentPermittedInvestment = watch('permittedInvestmentAllowedDTO')
          if (!currentPermittedInvestment) {
            const dtoId = typeof agreementParameter.permittedInvestmentAllowedDTO === 'object' 
              ? agreementParameter.permittedInvestmentAllowedDTO.id 
              : agreementParameter.permittedInvestmentAllowedDTO
            setValue('permittedInvestmentAllowedDTO', { id: dtoId }, {
              shouldValidate: true,
              shouldDirty: false,
            })
          }
        ...
        if (agreementParameter?.amendmentAllowedDTO) {
          ...
          if (!currentAmendment) {
            const dtoId = typeof agreementParameter.amendmentAllowedDTO === 'object' 
              ? agreementParameter.amendmentAllowedDTO.id 
              : agreementParameter.amendmentAllowedDTO
            setValue('amendmentAllowedDTO', { id: dtoId }, {
              shouldValidate: true,
              shouldDirty: false,
            })
          }
        ...
        if (agreementParameter?.dealClosureBasisDTO) {
          ...
          if (!currentClosureBasis) {
            const dtoId = typeof agreementParameter.dealClosureBasisDTO === 'object' 
              ? agreementParameter.dealClosureBasisDTO.id 
              : agreementParameter.dealClosureBasisDTO
            setValue('dealClosureBasisDTO', { id: dtoId }, {
              shouldValidate: true,
              shouldDirty: false,
            })
          }
        ...
        if (agreementParameter?.escrowAgreementDTO) {
          ...
          if (!currentEscrowAgreement) {
            setValue('escrowAgreementDTO', agreementParameter.escrowAgreementDTO, {
              shouldValidate: true,
              shouldDirty: false,
            })
          }
  ...
  const getDisplayLabel = (option: unknown, fallback: string) => {
    ...
    if (option && typeof option === 'object' && 'languageTranslationId' in option) {
      const translation = (option as { languageTranslationId?: { configValue?: string } }).languageTranslationId
  ...
  const renderDatePicker = (
    ...
    const minDate = isExpiryDate && effectiveDate ? effectiveDate.add(1, 'day') : null
          ...
          rules={{
            ...
            validate: (value: unknown) => {
              try {
                ...
                if (name === 'agreementEffectiveDate' || name === 'agreementExpiryDate') {
                  ...
                  if (isExpiryDate && effectiveDate && effectiveDate.isValid()) {
                    if (dateValue.isBefore(effectiveDate) || dateValue.isSame(effectiveDate)) {
                ...
                if (name === 'permittedInvestmentAllowedDTO' || 
              ...
              <DatePicker
                ...
                slotProps={{
                  textField: {
                    ...
                    sx: {
                      ...(typeof datePickerFieldStyles === 'object' ? datePickerFieldStyles : {}),
                      ...(!!errors[name] &&
                        !isReadOnly && {
                          '& .MuiOutlinedInput-root': {
                            '& fieldset': {
      ...
      <Grid key={`field-${name}`} size={{ xs: 12, md: gridSize }}>
        <Controller
          ...
          render={({ field, fieldState: { error } }) => {
            ...
            const fieldValue = typeof field.value === 'object' && field.value !== null && 'id' in field.value
            ...
            <FormControl fullWidth error={!!error} required={required}>
              ...
              >
                  {options.map((option, index) => {
                    const optionObj = option as { id?: string | number; settingValue?: string; languageTranslationId?: { configValue?: string } }
                    return (
                  <MenuItem
                  ...
                  >
                    {getDisplayLabel(
                          option,
                          optionObj.settingValue || optionObj.languageTranslationId?.configValue || 'Option'
                    )}
                  </MenuItem>
                    )
                  })}
