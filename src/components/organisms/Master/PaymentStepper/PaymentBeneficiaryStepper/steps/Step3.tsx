...
const Step3 = ({ paymentBeneficiaryId: propPaymentBeneficiaryId, onEditStep, isReadOnly = false }: Step3Props) => {
  const params = useParams()
  const paymentBeneficiaryId = propPaymentBeneficiaryId || (params.id as string)
  const isDarkMode = useIsDarkMode()

  const [paymentBeneficiaryDetails, setPaymentBeneficiaryDetails] = useState<PaymentBeneficiary | null>(
    null
  )
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      if (!paymentBeneficiaryId || paymentBeneficiaryId.trim() === '') {
        console.warn('[Step3] Payment Beneficiary ID is missing or empty:', paymentBeneficiaryId)
