...
export default function PaymentInstructionStepperWrapper({
  ...
  const stepConfigs = useMemo(
    () => ({
      ...
      2: {
        validateStep: async () => {
          if (!stepStatus?.step1) {
            notifications.showError(
              "Please complete Payment Instruction details before continuing."
            );
            return false;
          }
          return true;
    ...
    [validateFormFields, notifications, paymentInstructionId, stepStatus?.step1]
  );
  ...
  useEffect(() => {
    if (!paymentInstructionId || !stepStatus) return;
    ...
    const querySuffix = isViewMode
      ...
      : isEditingMode
        ...
        : "";
    router.replace(
      `/payment-instruction/${paymentInstructionId}/step/${maxAllowedStep + 1}${querySuffix}`
    );
