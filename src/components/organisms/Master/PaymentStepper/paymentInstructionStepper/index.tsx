...
import { usePaymentInstructionStepStatus, usePaymentInstructionStepManager } from '@/hooks/master/PaymentHook'
...
import type { PaymentInstructionDetailsData, PaymentInstruction, StepSaveResponse } from '@/services/api/masterApi/Payment/paymentInstructionService'
import {
...
export default function PaymentInstructionStepperWrapper({
  ...
  const paymentInstructionId = propPaymentInstructionId || standingInstructionId || (params.id as string) || ''
  ...
  const { data: stepStatus } = usePaymentInstructionStepStatus(
    paymentInstructionId && paymentInstructionId.trim() !== '' ? paymentInstructionId : ''
  )
  ...
  const getStepContent = useCallback(
    (step: number) => {
      switch (step) {
        case 0:
          return <Step1 isReadOnly={isViewMode} paymentInstructionId={paymentInstructionId} />
        case 1:
          return (
    ...
    [paymentInstructionId, isViewMode, handleEditStep, handleDocumentsChange, activeStep]
  )
    ...
    if (
      ...
      try {
        ...
        if (activeStep === 0 && processedData) {
          const dateFields = ['firstTransactionDateTime', 'instructionExpiryDateTime', 'nextExecutionDateTime']
    ...
    try {
      ...
      if (activeStep === 1) {
        ...
        if (nextStep < steps.length) {
          ...
          if (!paymentInstructionId) {
            notifications.showError('Payment Instruction ID is required to proceed to Review step.')
            setIsSaving(false)
            return
          }
      ...
      if (activeStep === 2) {
        ...
        try {
          ...
          const finalPaymentInstructionId = paymentInstructionId || paymentInstructionIdFromStatus
      ...
      if (activeStep === 0) {
        ...
        const convertDateToString = (dateValue: unknown): string | null | undefined => {
          if (!dateValue || dateValue === null || dateValue === '') return undefined
          ...
          if (dateValue && typeof dateValue === 'object' && 'format' in dateValue) {
            return (dateValue as { format: (format: string) => string }).format('YYYY-MM-DDTHH:mm:ss.SSS[Z]')
          }
          return undefined
        }

        const firstTransactionDateTime = convertDateToString(currentFormData.firstTransactionDateTime)
        const instructionExpiryDateTime = convertDateToString(currentFormData.instructionExpiryDateTime)
        const nextExecutionDateTime = convertDateToString(currentFormData.nextExecutionDateTime)

        const step1Data: PaymentInstructionDetailsData = {
          standingInstructionReferenceNumber: currentFormData.standingInstructionReferenceNumber as string,
          ...
          ...(firstTransactionDateTime !== undefined && { firstTransactionDateTime: firstTransactionDateTime ?? null }),
          ...(instructionExpiryDateTime !== undefined && { instructionExpiryDateTime: instructionExpiryDateTime ?? null }),
          retryIntervalDays: currentFormData.retryIntervalDays as number,
          retryUntilMonthEndFlag: currentFormData.retryUntilMonthEndFlag as boolean,
          ...(currentFormData.instructionRemarks !== undefined && { instructionRemarks: currentFormData.instructionRemarks as string }),
          ...(nextExecutionDateTime !== undefined && { nextExecutionDateTime: nextExecutionDateTime ?? null }),
          ...(currentFormData.dealNoDTO !== undefined && { dealNoDTO: currentFormData.dealNoDTO as { id: number } | number | null }),
          ...(currentFormData.statusDTO !== undefined && { statusDTO: currentFormData.statusDTO as { id: number } | number | null }),
          ...(currentFormData.transferTypeDTO !== undefined && { transferTypeDTO: currentFormData.transferTypeDTO as { id: number } | number | null }),
          ...(currentFormData.occurrenceDTO !== undefined && { occurrenceDTO: currentFormData.occurrenceDTO as { id: number } | number | null }),
          ...(currentFormData.recurringFrequencyDTO !== undefined && { recurringFrequencyDTO: currentFormData.recurringFrequencyDTO as { id: number } | number | null }),
          ...(currentFormData.holidaySetupDTO !== undefined && { holidaySetupDTO: currentFormData.holidaySetupDTO as { id: number } | number | null }),
          ...(currentFormData.dependentScenarioDTO !== undefined && { dependentScenarioDTO: currentFormData.dependentScenarioDTO as { id: number } | number | null }),
          ...(currentFormData.formAccountDrDTO !== undefined && { formAccountDrDTO: currentFormData.formAccountDrDTO as { id: number } | number | null }),
          ...(currentFormData.dependenceDTO !== undefined && { dependenceDTO: currentFormData.dependenceDTO as string | null }),
          ...(currentFormData.paymentTypeDTO !== undefined && { paymentTypeDTO: currentFormData.paymentTypeDTO as { id: number } | number | null }),
          ...(currentFormData.toAccountDTO !== undefined && { toAccountDTO: currentFormData.toAccountDTO as { id: number } | number | null }),
          ...(currentFormData.swiftCode !== undefined && { swiftCode: currentFormData.swiftCode as string | null }),
          ...(currentFormData.creditAmountCap !== undefined && { creditAmountCap: currentFormData.creditAmountCap as number }),
          ...(currentFormData.creditAmount !== undefined && { creditAmount: currentFormData.creditAmount as number }),
          ...(currentFormData.priority !== undefined && { priority: currentFormData.priority as number }),
          ...(currentFormData.recentPercentage !== undefined && { recentPercentage: currentFormData.recentPercentage as number }),
          ...(currentFormData.beneficiaryNameDTO !== undefined && { beneficiaryNameDTO: currentFormData.beneficiaryNameDTO as { id: number } | number | null }),
          ...(currentFormData.resetCounterDTO !== undefined && { resetCounterDTO: currentFormData.resetCounterDTO as { id: number } | number | null }),
        }
      ...
      if (activeStep < steps.length - 1) {
        if (activeStep === 0) {
          const saveResponseData = saveResponse as PaymentInstruction | StepSaveResponse
