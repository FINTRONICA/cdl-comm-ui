...
const Step1 = ({ isReadOnly = false }: Step1Props) => {
  ...
  const fieldStyles = React.useMemo(
    () => (typeof sharedCommonFieldStyles === 'function' ? sharedCommonFieldStyles(theme) : sharedCommonFieldStyles),
    [theme]
  )
  const dateFieldStyles = React.useMemo(
    () => (typeof sharedDatePickerStyles === 'function' ? sharedDatePickerStyles(theme) : sharedDatePickerStyles),
    [theme]
  )
  const labelStyles = React.useMemo(
    () => (typeof sharedLabelSx === 'function' ? sharedLabelSx(theme) : sharedLabelSx),
    [theme]
  )
  const valueStyles = React.useMemo(
    () => (typeof sharedValueSx === 'function' ? sharedValueSx(theme) : sharedValueSx),
    [theme]
  )
  const cardBaseStyles = React.useMemo(
    () => (typeof sharedCardStyles === 'function' ? (sharedCardStyles as (theme: Theme) => unknown)(theme) : sharedCardStyles),
    [theme]
  )
