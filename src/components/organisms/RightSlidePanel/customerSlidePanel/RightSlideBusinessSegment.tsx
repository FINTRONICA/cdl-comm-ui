...
const DEFAULT_VALUES: BusinessSegmentFormData = {
  businessSegmentId: '',
  businessSegmentName: '',
...
> = ({ isOpen, onClose, mode = 'add', actionData }) => {
  ...
  const {
    ...
    clearErrors,
  } = useForm<BusinessSegmentFormData>({
  ...
  useEffect(() => {
    ...
    const values: BusinessSegmentFormData =
      mode === 'edit' && actionData
        ? {
            businessSegmentId: (actionData as any).businessSegmentId ?? '',
            businessSegmentName: (actionData as any).businessSegmentName ?? '',
            businessSegmentDescription: (actionData as any).businessSegmentDescription ?? '',
          }
        : DEFAULT_VALUES
  ...
  const onSubmit = (data: BusinessSegmentFormData) => {
    if (isSubmitting) {
      return
    }
    ...
    if (mode === 'edit') {
      ...
      const updatePayload = {
        id: actionData.id,
        businessSegmentId: data.businessSegmentId.trim(),
        businessSegmentName: data.businessSegmentName.trim(),
        businessSegmentDescription: data.businessSegmentDescription.trim(),
      }
      ...
      updateAction.mutate(
        ...
        {
          onSuccess: () => {
            setSuccessMessage('Business Segment updated successfully.')
            setTimeout(() => {
              onClose()
            }, 1000)
          },
          onError: (err: Error | unknown) => {
            ...
            const message =
              ...
              error?.message ||
              'Failed to update business segment'
      ...
      const emptyFields = []
      if (!createPayload.businessSegmentId?.trim()) emptyFields.push('businessSegmentId')
      ...
      createAction.mutate(createPayload as any, {
        onSuccess: () => {
          setSuccessMessage('Business Segment created successfully.')
          setTimeout(() => {
            onClose()
          }, 1000)
        },
        onError: (err: Error | unknown) => {
          ...
          const message =
            error?.response?.data?.message ||
            error?.message ||
            'Failed to create business segment'
          setErrorMessage(message)
        },
      })
    }
  ...
  const renderTextField = (
    name: keyof BusinessSegmentFormData,
    label: string,
  ...
  const handleResetToLoaded = useCallback(() => {
    const loaded: BusinessSegmentFormData =
      mode === 'edit' && actionData
        ? {
            businessSegmentId: (actionData as any).businessSegmentId ?? '',
            businessSegmentName: (actionData as any).businessSegmentName ?? '',
  ...
  const onError = (errors: FieldErrors<BusinessSegmentFormData>) => {
    console.log(errors)
    ...
    >
      <Box sx={{ p: 3, borderBottom: '1px solid #E5E7EB' }}>
        ...
        >
          ...
          >
            {mode === 'edit' ? 'Edit Business Segment' : 'Add Business Segment'}
          </DialogTitle>
      ...
      <form onSubmit={handleSubmit(onSubmit, onError)}>
        <DialogContent dividers>
          ...
          <Grid container rowSpacing={4} columnSpacing={2} mt={3}>
            {renderTextField(
              'businessSegmentId',
              getBusinessSegmentLabel('CDL_CS_BUSINESS_SEGMENT_ID'),
              'text',
              12
            )}
            {renderTextField(
              'businessSegmentName',
              getBusinessSegmentLabel('CDL_CS_BUSINESS_SEGMENT_NAME'),
              'text',
              12
            )}
            {renderTextField(
              'businessSegmentDescription',
              getBusinessSegmentLabel('CDL_CS_BUSINESS_SEGMENT_DESCRIPTION'),
              'text',
              12,
              false
            )}
