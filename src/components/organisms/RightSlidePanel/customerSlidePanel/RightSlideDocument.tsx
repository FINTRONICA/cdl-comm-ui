...
interface RightSlideDocumentPanelProps {
  isOpen: boolean
  onClose: () => void
  mode?: 'add' | 'edit' | 'view'
...
export const RightSlideDocumentPanel: React.FC<
  RightSlideDocumentPanelProps
> = ({ isOpen, onClose, mode = 'add', actionData }) => {
  ...
  useEffect(() => {
    ...
    const values: DocumentFormData =
      mode === 'edit' && actionData
        ? {
            documentId: (actionData as any).documentId ?? '',
            documentName: (actionData as any).documentName ?? '',
            documentDescription: (actionData as any).documentDescription ?? '',
            documentType: (actionData as any).documentType ?? '',
            isRequired: (actionData as any).isRequired ?? false,
          }
        : DEFAULT_VALUES
  ...
  const onSubmit = (data: DocumentFormData) => {
    ...
    if (mode === 'edit') {
      ...
      if (!createPayload.documentType?.trim()) emptyFields.push('documentType')
  ...
  const onError = (errors: FieldErrors<DocumentFormData>) => {
    console.log(errors)
      ...
      <form onSubmit={handleSubmit(onSubmit, onError)}>
        <DialogContent dividers>
          ...
          <Grid container rowSpacing={4} columnSpacing={2} mt={3}>
            {renderTextField(
              'documentId',
              getDocumentLabel('CDL_CS_DOCUMENT_ID'),
              'text',
              12
            )}
            {renderTextField(
              'documentName',
              getDocumentLabel('CDL_CS_DOCUMENT_NAME'),
              'text',
              12
            )}
            {renderTextField(
              'documentType',
              getDocumentLabel('CDL_CS_DOCUMENT_TYPE'),
              'text',
              12
            )}
            {renderTextField(
              'documentDescription',
              getDocumentLabel('CDL_CS_DOCUMENT_DESCRIPTION'),
              'text',
              12,
              false
            )}
