...
interface RightSlideAccountPanelProps {
  isOpen: boolean
  onClose: () => void
  mode?: 'add' | 'edit' | 'view'
...
const DEFAULT_VALUES: AccountPurposeFormData = {
  accountPurposeId: '',
...
export const RightSlideAccountPanel: React.FC<
  RightSlideAccountPanelProps
> = ({ isOpen, onClose, mode = 'add', actionData }) => {
  ...
  const {
    ...
    clearErrors,
  } = useForm<AccountPurposeFormData>({
  ...
  useEffect(() => {
    ...
    const values: AccountPurposeFormData =
      mode === 'edit' && actionData
        ? {
            accountPurposeId: (actionData as any).accountPurposeId ?? '',
            accountPurposeShortName: (actionData as any).accountPurposeShortName ?? '',
            accountPurposeFullName: (actionData as any).accountPurposeFullName ?? '',
            criticality: (actionData as any).criticality ?? '',
          }
        : DEFAULT_VALUES
  ...
  const onSubmit = (data: AccountPurposeFormData) => {
    if (isSubmitting) {
      return
    }
    ...
    if (mode === 'edit') {
      ...
      const updatePayload = {
        id: actionData.id,
        accountPurposeId: data.accountPurposeId.trim(),
        accountPurposeShortName: data.accountPurposeShortName.trim(),
        accountPurposeFullName: data.accountPurposeFullName.trim(),
        criticality: data.criticality.trim(),
      }
      ...
      updateAction.mutate(
        ...
        {
          onSuccess: () => {
            setSuccessMessage('Account Purpose updated successfully.')
            setTimeout(() => {
              onClose()
            }, 1000)
          },
          onError: (err: Error | unknown) => {
            ...
            const message =
              ...
              error?.message ||
              'Failed to update account purpose'
      ...
      const emptyFields = []
      if (!createPayload.accountPurposeId?.trim()) emptyFields.push('accountPurposeId')
      if (!createPayload.accountPurposeShortName?.trim()) emptyFields.push('accountPurposeShortName')
      if (!createPayload.accountPurposeFullName?.trim()) emptyFields.push('accountPurposeFullName')
      if (!createPayload.criticality?.trim()) emptyFields.push('criticality')
      ...
      createAction.mutate(createPayload as any, {
        onSuccess: () => {
          setSuccessMessage('Account Purpose created successfully.')
          setTimeout(() => {
            onClose()
          }, 1000)
        },
        onError: (err: Error | unknown) => {
          ...
          const message =
            error?.response?.data?.message ||
            error?.message ||
            'Failed to create account purpose'
          setErrorMessage(message)
        },
      })
    }
  ...
  const renderTextField = (
    name: keyof AccountPurposeFormData,
    label: string,
  ...
  const handleResetToLoaded = useCallback(() => {
    const loaded: AccountPurposeFormData =
      mode === 'edit' && actionData
        ? {
            accountPurposeId: (actionData as any).accountPurposeId ?? '',
            accountPurposeShortName: (actionData as any).accountPurposeShortName ?? '',
  ...
  const onError = (errors: FieldErrors<AccountPurposeFormData>) => {
    console.log(errors)
    ...
    >
      <Box sx={{ p: 3, borderBottom: '1px solid #E5E7EB' }}>
        ...
        >
          ...
          >
            {mode === 'edit' ? 'Edit Account Purpose' : 'Add Account Purpose'}
          </DialogTitle>
      ...
      <form onSubmit={handleSubmit(onSubmit, onError)}>
        <DialogContent dividers>
          ...
          <Grid container rowSpacing={4} columnSpacing={2} mt={3}>
            {renderTextField(
              'accountPurposeId',
              getAccountPurposeLabel('CDL_CS_ACCOUNT_PURPOSE_ID'),
              'text',
              12
            )}
            {renderTextField(
              'accountPurposeShortName',
              getAccountPurposeLabel('CDL_CS_ACCOUNT_PURPOSE_SHORT_NAME'),
              'text',
              12
            )}
            {renderTextField(
              'accountPurposeFullName',
              getAccountPurposeLabel('CDL_CS_ACCOUNT_PURPOSE_FULL_NAME'),
              'text',
              12
            )}
            {renderTextField(
              'criticality',
              getAccountPurposeLabel('CDL_CS_ACCOUNT_PURPOSE_CRITICALITY'),
              'text',
              12
            )}
          </Grid>
