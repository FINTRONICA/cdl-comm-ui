...
interface RightSlideInvestmentPanelProps {
  isOpen: boolean
  onClose: () => void
  mode?: 'add' | 'edit' | 'view'
...
const DEFAULT_VALUES: InvestmentFormData = {
  investmentId: '',
  investmentName: '',
  investmentDescription: '',
}

export const RightSlideInvestmentPanel: React.FC<
  RightSlideInvestmentPanelProps
> = ({ isOpen, onClose, mode = 'add', actionData }) => {
  ...
  useEffect(() => {
    ...
    const values: InvestmentFormData =
      mode === 'edit' && actionData
        ? {
            investmentId: actionData.investmentId ?? '',
            investmentName: actionData.investmentName ?? '',
            investmentDescription: actionData.investmentDescription ?? '',
          }
        : DEFAULT_VALUES
  ...
  const onSubmit = (data: InvestmentFormData) => {
    ...
    if (mode === 'edit') {
      ...
      const updatePayload = {
        id: actionData.id,
        investmentId: data.investmentId.trim(),
        investmentName: data.investmentName.trim(),
        investmentDescription: data.investmentDescription.trim(),
      }
      ...
      const emptyFields = []
      if (!createPayload.investmentId?.trim()) emptyFields.push('investmentId')
      if (!createPayload.investmentName?.trim()) emptyFields.push('investmentName')
      ...
      // @ts-ignore - Custom payload structure for investment data
  ...
  const handleResetToLoaded = useCallback(() => {
    const loaded: InvestmentFormData =
      mode === 'edit' && actionData
        ? {
            investmentId: actionData.investmentId ?? '',
            investmentName: actionData.investmentName ?? '',
      ...
      <form onSubmit={handleSubmit(onSubmit, onError)}>
        <DialogContent dividers>
          ...
          <Grid container rowSpacing={4} columnSpacing={2} mt={3}>
            {renderTextField(
              'investmentId',
              getInvestmentLabel('CDL_CS_INVESTMENT_ID'),
              'text',
              12
            )}
            {renderTextField(
              'investmentName',
              getInvestmentLabel('CDL_CS_INVESTMENT_NAME'),
              'text',
              12
            )}
            {renderTextField(
              'investmentDescription',
              getInvestmentLabel('CDL_CS_INVESTMENT_DESCRIPTION'),
              'text',
              12,
              false
            )}
