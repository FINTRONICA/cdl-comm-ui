...
> = ({
  ...
  const {
    ...
    GeneralLedgerAccountFormData & { generalLedgerAccountId?: string }
  >({
    defaultValues: {
  ...
  useEffect(() => {
    ...
    const currentId =
      (apiGeneralLedgerAccountData?.id || actionData?.id) ?? null;
    const shouldReset =
    ...
    if (mode === "edit") {
      ...
      if (shouldReset && (apiGeneralLedgerAccountData || actionData)) {
        ...
        const generalLedgerAccountId =
          dataToUse.uuid || `GLA-${dataToUse.id}` || "";
  ...
  const validateGeneralLedgerAccountField = useCallback(
    (
      ...
      value: unknown,
      allValues: GeneralLedgerAccountFormData & {
      ...
      try {
        const requiredFields: Record<string, string> = {
          ...
          ledgerAccountDescription:
            "General Ledger Account Description is required",
          ...
          rules={{
            ...
            validate: (value, formValues) => {
              ...
              return validateGeneralLedgerAccountField(
                fieldName,
                value,
                formValues as GeneralLedgerAccountFormData & {
                  generalLedgerAccountId?: string;
                }
              );
            },
  ...
  const onSubmit = useCallback(
    async (
      data: GeneralLedgerAccountFormData & { generalLedgerAccountId?: string }
    ) => {
      try {
        ...
        if (!isEditing && !data.generalLedgerAccountId && !generatedId) {
          setErrorMessage(
            "Please generate a General Ledger Account ID before submitting."
          );
          return;
        }

        const generalLedgerAccountData:
          | CreateGeneralLedgerAccountRequest
          | UpdateGeneralLedgerAccountRequest = {
        ...
        if (
          ...
          onGeneralLedgerAccountUpdated(
            result as GeneralLedgerAccount,
            generalLedgerAccountIndex
          );
        ...
        let errorMessage =
          "Failed to save general ledger account. Please try again.";
