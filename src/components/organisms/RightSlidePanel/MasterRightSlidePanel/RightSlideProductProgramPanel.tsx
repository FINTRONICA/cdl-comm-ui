...
> = ({
  ...
  React.useEffect(() => {
    ...
    if (mode === "edit") {
      ...
      if (shouldReset && (apiProductProgramData || actionData)) {
        ...
        const tableData = dataToUse as ProductProgram & {
          productProgramName?: string;
          productProgramDescription?: string;
        };
        const programName =
          tableData.programName || tableData.productProgramName || "";
        const programDescription =
          tableData.programDescription ||
          tableData.productProgramDescription ||
          "";
  ...
  const onSubmit = async (
    data: ProductProgramFormData & { productProgramId?: string }
  ) => {
    try {
      ...
      if (!isEditing && !data.productProgramId && !generatedId) {
        setErrorMessage(
          "Please generate a Product Program ID before submitting."
        );
        return;
      }
      ...
      if (!isValid) {
        ...
        if (!data.programDescription)
          errors.push("Product Program Description is required");
        if (errors.length > 0) {
          setErrorMessage(
      ...
      const productProgramId = isEditing
    ...
    <Grid key={name} size={{ xs: 12, md: gridSize }}>
      <Controller
        ...
        rules={{
          ...
          validate: (value, formValues) =>
            validateProductProgramField(
              name,
              value,
              formValues as ProductProgramFormData & {
            ...
            <Grid container spacing={2}>
              <Grid size={{ xs: 6 }}>
                <Button
                  ...
                  onClick={handleClose}
                  disabled={
                    saveProductProgramMutation.isPending || taskStatusLoading
                  }
                  sx={{
                    fontFamily: "Outfit, sans-serif",
                    fontWeight: 500,
