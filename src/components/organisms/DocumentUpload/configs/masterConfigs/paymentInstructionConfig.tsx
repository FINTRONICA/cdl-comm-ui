...
// Service adapter for payment instruction documents
export const paymentInstructionDocumentService: DocumentService<
  DocumentItem,
  ApiDocumentResponse
> = {
  getDocuments: async (paymentInstructionId: string, page = 0, size = 20) => {
    return paymentInstructionService.getPaymentInstructionDocuments(
  ...
  uploadDocument: async (
    ...
    return paymentInstructionService.uploadPaymentInstructionDocument(
      file,
      paymentInstructionId,
      'PAYMENT_INSTRUCTION',
      documentType
...
export const mapApiToPaymentInstructionDocumentItem = (
...
// Table columns configuration for payment instruction documents
export const paymentInstructionColumns: TableColumn<DocumentItem>[] = [
  {
    key: 'name',
...
// Document actions configuration for payment instruction documents
export const paymentInstructionActions: DocumentAction<DocumentItem>[] = [
  {
    key: 'download',
    label: 'Download',
  ...
  },
  {
    ...
    onClick: async (_document: DocumentItem) => {
      ...
      // as it requires updating the parent component
      // as it requires updating the local state
    },
  },
]

// Factory function to create payment instruction document upload configuration
export const createPaymentInstructionDocumentConfig = (
  paymentInstructionId: string,
  options?: {
    title?: string
    description?: string
  ...
  const actions = [...paymentInstructionActions]
  if (options?.onDelete) {
  ...
  const config: DocumentUploadConfig<DocumentItem, ApiDocumentResponse> = {
    ...
    documentService: paymentInstructionDocumentService,
    mapApiToDocument: mapApiToPaymentInstructionDocumentItem,
    documentTypeSettingKey: 'PAYMENT_INSTRUCTION_ID_TYPE', // Setting key for payment instruction document types (using standard document type key)
    title: options?.title || 'Payment Instruction Documents',
    description:
      options?.description ||
      'This step is optional. You can upload payment instruction-related documents or skip to continue.',
    isOptional: options?.isOptional ?? true,
    isReadOnly: options?.isReadOnly ?? false,
    columns: paymentInstructionColumns,
    actions,
  }
