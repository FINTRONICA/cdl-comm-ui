...
// Service adapter for beneficiary documents
export const beneficiaryDocumentService: DocumentService<
  DocumentItem,
  ApiDocumentResponse
> = {
  getDocuments: async (beneficiaryId: string, page = 0, size = 20) => {
    return beneficiaryService.getBeneficiaryDocuments(
      beneficiaryId,
      'BENEFICIARY',
      page,
      size
  ...
  uploadDocument: async (
    ...
    return beneficiaryService.uploadBeneficiaryDocument(
      file,
      beneficiaryId,
      'BENEFICIARY',
      documentType
...
export const mapApiToBeneficiaryDocumentItem = (
...
// Table columns configuration for beneficiary documents
export const beneficiaryColumns: TableColumn<DocumentItem>[] = [
  {
    key: 'name',
    label: 'Document Name',
    width: '40%',
...
// Document actions configuration for beneficiary documents
export const beneficiaryActions: DocumentAction<DocumentItem>[] = [
  // {
  //   key: 'view',
  //   label: 'View',
...
// Factory function to create beneficiary document upload configuration
export const createBeneficiaryDocumentConfig = (
  beneficiaryId: string,
  options?: {
    title?: string
    description?: string
  ...
  const actions = [...beneficiaryActions]
  if (options?.onDelete) {
  ...
  const config: DocumentUploadConfig<DocumentItem, ApiDocumentResponse> = {
    ...
    documentService: beneficiaryDocumentService,
    mapApiToDocument: mapApiToBeneficiaryDocumentItem,
    documentTypeSettingKey: 'BENEFICIARY_ID_TYPE',
    title: options?.title || 'Beneficiary Documents',
    description:
      options?.description ||
      'This step is optional. You can upload beneficiary-related documents or skip to continue.',
