...
import {
  DocumentItem,
  ApiDocumentResponse,
} from '../../../Master/PartyStepper/steps/partyTypes'
...
// Service adapter for party documents
export const partyDocumentService: DocumentService<
  DocumentItem,
  ApiDocumentResponse
> = {
  getDocuments: async (partyId: string, page = 0, size = 20) => {
    return partyService.getPartyDocuments(
      partyId,
      'PARTY',
      page,
      size
  ...
  uploadDocument: async (
    ...
    return partyService.uploadPartyDocument(
      file,
      partyId,
      'PARTY',
      documentType
...
// Table columns configuration for party documents
export const partyColumns: TableColumn<DocumentItem>[] = [
  {
    key: 'name',
...
// Document actions configuration for party documents
export const partyActions: DocumentAction<DocumentItem>[] = [
  // {
  //   key: 'view',
  //   label: 'View',
...
// Factory function to create party document upload configuration
export const createPartyDocumentConfig = (
  partyId: string,
  options?: {
    title?: string
    description?: string
  ...
  const config: DocumentUploadConfig<DocumentItem, ApiDocumentResponse> = {
    entityId: partyId,
    entityType: 'PARTY',
    documentService: partyDocumentService,
    mapApiToDocument: mapApiToDocumentItem,
    documentTypeSettingKey: 'INVESTOR_ID_TYPE', // Default setting key for party documents
