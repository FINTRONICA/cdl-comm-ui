...
// Service adapter for account documents
export const accountDocumentService: DocumentService<
  DocumentItem,
  ApiDocumentResponse
> = {
  getDocuments: async (accountId: string, page = 0, size = 20) => {
    return accountService.getAccountDocuments(
      accountId,
      'ACCOUNT',
      page,
      size
  ...
  uploadDocument: async (
    ...
    return accountService.uploadAccountDocument(
      file,
      accountId,
      'ACCOUNT',
      documentType
...
export const mapApiToAccountDocumentItem = (
...
// Table columns configuration for account documents
export const accountColumns: TableColumn<DocumentItem>[] = [
  {
    key: 'name',
...
// Document actions configuration for account documents
export const accountActions: DocumentAction<DocumentItem>[] = [
  {
    key: 'download',
    label: 'Download',
...
// Factory function to create account document upload configuration
export const createAccountDocumentConfig = (
  accountId: string,
  options?: {
    title?: string
    description?: string
  ...
  const config: DocumentUploadConfig<DocumentItem, ApiDocumentResponse> = {
    ...
    documentTypeSettingKey: 'ACCOUNT_DOCUMENT_TYPE', // Setting key for account document types
    title: options?.title || 'Account Documents',
    description:
      options?.description ||
      'This step is optional. You can upload account-related documents or skip to continue.',
