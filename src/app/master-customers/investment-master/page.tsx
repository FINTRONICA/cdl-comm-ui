...
import { getLabelByConfigId as getInvestmentLabel } from '@/constants/mappings/customerMapping'
import { useAppStore } from '@/store'
import { Spinner } from '@/components/atoms/Spinner'
import { RightSlideInvestmentPanel } from '@/components/organisms/RightSlidePanel/customerSlidePanel/RightSlideInvestment'
...
const InvestmentPageImpl: React.FC = () => {
  ...
  const investmentData = useMemo(() => [
    {
      ...
      investmentDescription: 'Low-risk government treasury bonds',
    },
    {
      id: 2,
      investmentId: 'INV002',
      investmentName: 'Corporate Bonds',
      investmentDescription: 'Medium-risk corporate debt securities',
    },
    {
      id: 3,
      investmentId: 'INV003',
      investmentName: 'Equity Funds',
      investmentDescription: 'Diversified equity mutual funds',
    },
    {
      id: 4,
      investmentId: 'INV004',
      investmentName: 'Real Estate Investment Trusts',
      investmentDescription: 'REIT investments in commercial properties',
    },
    {
      id: 5,
      investmentId: 'INV005',
      investmentName: 'Commodity Funds',
      investmentDescription: 'Investment funds focused on commodities',
    },
  ] as InvestmentData[], [])

  const getInvestmentLabelDynamic = useCallback(
    (configId: string): string => {
      return getInvestmentLabel(configId)
    },
    []
  )
  ...
  const tableColumns = [
    {
      key: 'investmentId',
      label: getInvestmentLabelDynamic('CDL_CS_INVESTMENT_ID'),
      type: 'text' as const,
      width: 'w-48',
      sortable: true,
    },
    {
      key: 'investmentName',
      label: getInvestmentLabelDynamic('CDL_CS_INVESTMENT_NAME'),
      type: 'text' as const,
      width: 'w-48',
      sortable: true,
    },
    {
      ...
      label: getInvestmentLabelDynamic('CDL_CS_INVESTMENT_DESCRIPTION'),
      type: 'text' as const,
      width: 'w-64',
      sortable: true,
    },
  ...
  const handleRowView = (row: InvestmentData) => {
    setEditingItem(row)
    setPanelMode('edit')
    setIsPanelOpen(true)
  }
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div>
            <span className="text-gray-600">
              {getInvestmentLabelDynamic('CDL_CS_INVESTMENT_ID')}:
            </span>
          ...
          <div>
            <span className="text-gray-600">
              {getInvestmentLabelDynamic('CDL_CS_INVESTMENT_NAME')}:
            </span>
            <span className="ml-2 font-medium text-gray-800">
              {row.investmentName || '-'}
            </span>
          ...
          <div className="col-span-2">
            <span className="text-gray-600">
              {getInvestmentLabelDynamic('CDL_CS_INVESTMENT_DESCRIPTION')}:
            </span>
            <span className="ml-2 font-medium text-gray-800">
