    ...
    <div className="w-full max-w-md text-center">
      <div className="mb-8">
        ...
        <h1 className="mb-4 text-2xl font-semibold text-gray-900">
          Failed to load agreement parameters
        </h1>
...
const AgreementParametersPageImpl: React.FC = () => {
  ...
  const {
    data: apiResponse,
    isLoading: agreementParametersLoading,
    isFetching: agreementParametersFetching,
    error: agreementParametersError,
  ...
  const handleRowDelete = (row: AgreementParameterData) => {
    ...
    confirmDelete({
      itemName: `agreement parameter: ${row.id}`,
      itemId: row.id,
      onConfirm: async () => {
        try {
          setIsDeleting(true);
  ...
  const isRefreshLoading = isRefreshing || agreementParametersFetching;
  const showRefreshOverlay = isRefreshLoading || agreementParametersLoading;
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      await refetchAgreementParameters();
    } finally {
      setIsRefreshing(false);
    }
  }, [isRefreshing, refetchAgreementParameters]);
  if (agreementParametersLoading || agreementParametersFetching) {
    return (
      <DashboardLayout title={agreementParametersPageTitle}>
        <div className="flex flex-col h-full bg-white/75 dark:bg-gray-800/80 rounded-2xl">
          <GlobalLoading fullHeight />
        </div>
      </DashboardLayout>
    ...
    <>
      ...
      <DashboardLayout title={agreementParametersPageTitle}>
        <div className="relative flex flex-col h-full bg-white/75 dark:bg-gray-800/80 rounded-2xl">
          ...
          {agreementParametersLoading ? (
            <LoadingSpinner />
          ) : agreementParametersError ? (
            <div className="flex flex-col h-full bg-white/75 dark:bg-gray-800/80 rounded-2xl">
              <ErrorMessage
                error={agreementParametersError}
                onRetry={refetchAgreementParameters}
              />
            </div>
          ) : (
            <>
