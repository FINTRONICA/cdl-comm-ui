...
export const getAgreementParameterStepSchema = (stepNumber: number) => {
  ...
  if (stepIndex < 0 || stepIndex >= stepKeys.length) {
    console.warn(`[Validation] Invalid step number: ${stepNumber}, expected 1-3`)
    return null
  }
...
export const validateAgreementParameterStepData = (
  ...
  try {
    console.log(`[Validation] Validating step ${stepNumber} with data:`, data)
    
    const schema = getAgreementParameterStepSchema(stepNumber)
    if (!schema) {
      console.warn(`[Validation] No schema found for step ${stepNumber}`)
    ...
    console.log(`[Validation] Extracted step data for step ${stepNumber}:`, stepData)

    // Validate only the step-specific data
    const result = schema.safeParse(stepData)

    if (!result.success) {
      console.error(`[Validation] Step ${stepNumber} validation failed:`, result.error.issues)
    } else {
      console.log(`[Validation] Step ${stepNumber} validation passed`)
    }

    return result
  } catch (error) {
    console.error(`[Validation] Exception during step ${stepNumber} validation:`, error)
    return {
      success: false,
      error: {
