  ...
  return useMutation({
    mutationFn: async ({
      ...
      if (isEditing && investmentId) {
        return investmentService.updateInvestment(investmentId, data as UpdateInvestmentRequest)
      } else {
        return investmentService.createInvestment(data as CreateInvestmentRequest)
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: [INVESTMENTS_QUERY_KEY] })
