...
/**
 * Hook for consuming processed transaction label configuration API
 * Provides reactive access to processed transaction label data with caching and error handling
 */
export function useProcessedTransactionLabelApi() {
  ...
  const fetchLabels = useCallback(async () => {
    try {
      ...
      const rawLabels = await processedTransactionLabelsService.getProcessedTransactionLabelsWithCache()
...
/**
 * React Query hook for processed transaction labels
 * Provides caching, background updates, and error handling
 */
export function useProcessedTransactionLabelsQuery(language: string = 'EN') {
  return useQuery({
    queryKey: ['processed-transaction-labels', language],
    queryFn: () => processedTransactionLabelsService.getProcessedTransactionLabelsWithCache(),
...
/**
 * React Query hook for getting a specific processed transaction label
 */
export function useProcessedTransactionLabelQuery(
  configId: string,
  language: string = 'EN'
) {
  return useQuery({
    queryKey: ['processed-transaction-label', configId, language],
    queryFn: () => processedTransactionLabelsService.getLabel(configId, language),
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
    retry: 3,
  })
...
/**
 * React Query hook for getting processed transaction labels by module
 */
export function useProcessedTransactionLabelsByModuleQuery(
  moduleCode: string,
  language: string = 'EN'
) {
  return useQuery({
    queryKey: ['processed-transaction-labels-by-module', moduleCode, language],
    queryFn: () => processedTransactionLabelsService.getLabelsByModule(moduleCode, language),
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
    retry: 3,
  })
...
/**
 * React Query hook for getting processed transaction labels by language
 */
export function useProcessedTransactionLabelsByLanguageQuery(language: string) {
  return useQuery({
    queryKey: ['processed-transaction-labels-by-language', language],
    queryFn: () => processedTransactionLabelsService.getLabelsByLanguage(language),
    staleTime: 5 * 60 * 1000,
    gcTime: 10 * 60 * 1000,
    retry: 3,
  })
